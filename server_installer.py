"""
Автоустановка нового сервера сервиса (ноды AmneziaWG) прямо из веб-админки.
...
Все шаги, которые раньше делались руками по инструкции (см. инструкция.txt):
  ...
  3) проверка контейнера amnezia-awg2; если его нет - АВТОУСТАНОВКА по шаблону:
     ...
     дополнительных параметров протокола, ip_forward, NAT (MASQUERADE), запуск контейнера;
...
# Шаблон клиентских параметров протокола AmneziaWG (как в приложении Amnezia по умолчанию).
